...
func (module *setter) CreateBulkInvite(ctx context.Context, orgID valuer.UUID, identityID valuer.UUID, identityEmail valuer.Email, bulkInvites *types.PostableBulkInviteRequest) ([]*types.Invite, error) {
	...
	for idx, userWithToken := range newUsersWithResetToken {
		...
		if err := module.emailing.SendHTML(ctx, userWithToken.User.Email.String(), "You're invited: Voltusfreight", emailtypes.TemplateNameInvitationEmail, map[string]any{
...
func (module *setter) CreatePendingInviteUser(ctx context.Context, identityID valuer.UUID, identityEmail valuer.Email, frontendBaseURL string, user *types.User, opts ...root.CreateUserOption) (*types.User, error) {
	...
	if err := module.emailing.SendHTML(ctx, user.Email.String(), "You're invited: Voltusfreight", emailtypes.TemplateNameInvitationEmail, map[string]any{
		"inviter_email": identityEmail.StringValue(),
		"link":          resetLink,
		"Expiry":        humanizedTokenLifetime,
	}); err != nil {
